...
from ..client import app, limiter, enable_https
...
async def edit_target_info(request: Request, target_id: str):
    ...
    try:
        ...
        if plugins is not None and not isinstance(plugins, list):
            raise HTTPException(status_code=400, detail='"plugins" must be list')
...
@app.get("/api/plugins_list")
async def get_plugins_list(request: Request):
    try:
        verify_jwt(request)
        plugins_list = await JobQueueClient.get_plugins_list()
...
@app.get("/api/plugins")
async def get_plugins_info(request: Request, locale: str = Query(default_locale)):
    try:
        verify_jwt(request)
        plugins = await JobQueueClient.get_plugins_info(locale=locale)
...
@app.get("/api/plugin/{plugin_name}/related")
async def search_related_plugin(request: Request, plugin_name: str):
    try:
        verify_jwt(request)
        plugins = await JobQueueClient.get_plugin_related(plugin=plugin_name)
        return {"plugins": plugins}
    except HTTPException as e:
        raise e
    except Exception:
...
@app.get("/api/plugin/{plugin_name}/helpdoc")
async def get_plugin_helpdoc(request: Request, plugin_name: str, locale: str = Query(default_locale)):
    try:
        verify_jwt(request)
        help_doc = await JobQueueClient.get_plugin_helpdoc(plugin=plugin_name, locale=locale)
        if not help_doc:
            raise HTTPException(status_code=404, detail="Not found")
        return help_doc
    except HTTPException as e:
...
@app.post("/api/plugin/{plugin_name}/reload")
async def reload_plugin(request: Request, plugin_name: str):
    ip = request.client.host
    try:
        verify_jwt(request)
        status = await JobQueueClient.post_plugin_action(plugin=plugin_name, action="reload")
        if not status:
            Logger.warning(f"[WebUI] {ip} failed to reload plugin: {plugin_name}")
            raise HTTPException(status_code=422, detail="Reload plugins failed")
        Logger.info(f"[WebUI] {ip} has reloaded the plugin: {plugin_name}")
        return Response(status_code=204)
    except HTTPException as e:
        raise e
    except Exception:
...
@app.post("/api/plugin/{plugin_name}/load")
async def load_plugin(request: Request, plugin_name: str):
    ip = request.client.host
    try:
        verify_jwt(request)
        status = await JobQueueClient.post_plugin_action(plugin=plugin_name, action="load")
        if not status:
            Logger.warning(f"[WebUI] {ip} failed to load plugin: {plugin_name}")
            raise HTTPException(status_code=422, detail="Load plugins failed")
        Logger.info(f"[WebUI] {ip} has loaded the plugin: {plugin_name}")
        return Response(status_code=204)
...
@app.post("/api/plugin/{plugin_name}/unload")
async def unload_plugin(request: Request, plugin_name: str):
    ip = request.client.host
    try:
        verify_jwt(request)
        status = await JobQueueClient.post_plugin_action(plugin=plugin_name, action="unload")
        if not status:
            Logger.warning(f"[WebUI] {ip} failed to unload plugin: {plugin_name}")
            raise HTTPException(status_code=422, detail="Unload plugins failed")
        Logger.info(f"[WebUI] {ip} has unloaded the plugin: {plugin_name}")
        return Response(status_code=204)
    except HTTPException as e:
        raise e
    except Exception:
