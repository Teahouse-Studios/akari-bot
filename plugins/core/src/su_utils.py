...
su = plugin("superuser", alias="su", required_superuser=True, base=True, doc=True)
...
purge = plugin("purge", required_superuser=True, base=True, doc=True)
...
set_ = plugin("set", required_superuser=True, base=True, doc=True)
...
async def _(msg: Bot.MessageSession, target: str):
    ...
    if "enable" in msg.parsed_msg:
        modules = [
            ...
            if m
            in PluginsManager.return_plugins_list(
...
post_whitelist = plugin("post_whitelist", required_superuser=True, base=True, doc=True, available_for="QQ")
...
ae = plugin("abuse", alias="ae", required_superuser=True, base=True, doc=True)
...
upd = plugin("update", required_superuser=True, base=True, doc=True)
...
async def update_dependencies():
    returncode, _, uv_sync = await run_sys_command(["uv", "sync", "--all-extras"], timeout=60)
    if returncode == 0:
        return uv_sync
...
rst = plugin("restart", required_superuser=True, base=True, doc=True, exclude_from=["Web"], load=Bot.Info.subprocess)
...
resume = plugin("resume", required_base_superuser=True, base=True, doc=True, available_for="QQ")
...
forward_msg = plugin("forward_msg", required_superuser=True, base=True, doc=True)
...
echo = plugin("echo", required_superuser=True, base=True, doc=True)
...
say = plugin("say", required_superuser=True, base=True, doc=True)
...
rse = plugin("raise", required_superuser=True, base=True, doc=True)
...
post_ = plugin("post", required_superuser=True, base=True, doc=True)
...
cfg_ = plugin("config", required_superuser=True, alias="cfg", base=True, doc=True)
...
jobqueue = plugin("jobqueue", required_superuser=True, base=True)
...
decry = plugin("decrypt", required_superuser=True, base=True, doc=True)
...
wr = plugin("webrender", required_superuser=True, base=True)
