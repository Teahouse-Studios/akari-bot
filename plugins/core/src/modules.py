...
async def config_modules(msg: Bot.MessageSession):
    ...
    alias = PluginsManager.modules_aliases
    modules_ = PluginsManager.return_plugins_list(
    ...
    if msg.parsed_msg.get("enable", False):
            ...
            for module_ in wait_config_list:
                if module_ not in modules_:
                    ...
                    if not modules_[module_]._db_load:
                        ...
                        recommend = modules_[module_].recommend_plugins
                        if recommend:
                            for r in recommend:
            ...
            for m in recommend_modules_list:
                try:
                    ...
                    hdoc = CommandParser(
                        modules_[m],
                        msg=msg,
                        plugin_name=modules_[m].plugin_name,
        ...
        async def module_reload(module, extra_modules, base_module=False):
            status, reload_count = await PluginsManager.reload_plugin(module)
            if not status:
        ...
        for module_ in wait_config_list:
            ...
            if module_ not in modules_:
                ...
                extra_reload_modules = PluginsManager.search_related_plugin(module_, False)
        ...
        for module_ in wait_config_list:
            if module_ not in await ModuleStatus.get_unloaded_modules():
                ...
                continue
            if await PluginsManager.load_plugin(module_):
        ...
        for module_ in wait_config_list:
            ...
            if modules_[module_].base:
                ...
                continue
            if await PluginsManager.unload_plugin(module_):
