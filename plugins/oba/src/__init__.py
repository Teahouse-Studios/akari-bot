...
oba = plugin("oba", desc="{I18N:oba.help.desc}", developers="WorldHim")
...
async def _(msg: Bot.MessageSession, rank: int = 1):
    ...
    if "sponsor" not in node:
        ...
        await msg.send_message(msg_chain)
        ...
        try:
            await msg.finish([send_msg, Image(banner)])
        except Exception:
...
async def _(msg: Bot.MessageSession, rank: int = 1):
    ...
    node_list.append(I18NContext("message.collapse", amount=TOP_LIMIT))
