...
async def test_locale_set(tester: Tester):
    """locale 设置语言测试"""
    await tester.integrate("~locale en_us", Contains("语言"), "locale set 应提示语言设置")
    await tester.integrate("~locale zh_cn", Contains("语言"), "locale 恢复中文")
    return tester
...
async def test_version_sys(tester: Tester):
    """version sys 系统信息测试"""
    await tester.integrate("~version sys", Contains("系统"), "version sys 应显示系统信息")
    return tester
