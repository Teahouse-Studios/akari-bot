...
async def test_wiki_search(tester: Tester):
    """wiki search 搜索测试"""
    await tester.integrate("~wiki set https://zh.minecraft.wiki/api.php", Contains("成功设置起始 Wiki"), "设置 Wiki")
    await tester.integrate("~wiki search Minecraft", Contains("搜索"), "wiki search 应返回搜索结果")
    return tester
...
async def test_wiki_headers_manage(tester: Tester):
    """wiki headers 管理测试"""
    await tester.integrate("~wiki headers list", Contains("请求标头"), "wiki headers list 应显示标头")
    return tester
...
async def test_wiki_prefix_manage(tester: Tester):
    """wiki prefix 管理测试"""
    await tester.integrate("~wiki prefix Test", Contains("前缀"), "wiki prefix 应提示设置结果")
...
async def test_wiki_page_info(tester: Tester):
    """wiki page 页面信息测试"""
    await tester.integrate("~wiki page Minecraft", Contains("Minecraft"), "wiki page 应返回页面信息")
    return tester
