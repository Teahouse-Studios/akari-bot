                            ...
                            if 'players' in jejson:
                                ...
                                if showplayer:
                                    ...
                                    if 'sample' in jejson['players']:
                                        ...
                                        servers.append(lang.t('server.player.current') + '\n'.join(playerlist))
                                    else:
            ...
            async with aiohttp.ClientSession() as session2:
                async with session2.get(beurl, timeout=aiohttp.ClientTimeout(total=20)) as req:
                    if req.status != 200:
                        ...
                        bemsg = '[BE]\n' + \
                                motd_1 + ' - ' + motd_2 + \
                                f'\n{lang.t("server.player")}' + player_count + '/' + max_players + \
                                f'\n{lang.t("server.version")}' + edition + version_name + \
                                f'\n{lang.t("server.gamemode")}' + game_mode
