...
MAX_DETAIL_CNT = 1  # n次摇动的骰子的总量超过该值时将不再显示详细信息
...
class DiceValueError(Exception):
    ...
    def __init__(self, message , value):
        self.message = f"参数错误: {message}: {value}"
...
class DiceMod(DiceItemBase):
    ...
    def __init__(self, diceCode: str,postive: bool):
        ...
        if not diceCode.isdigit():
            ...
            self.value = int(diceCode)
            if self.value > MAX_MOD_NUMBER or self.value < MIN_MOD_NUMBER:
                raise DiceValueError(f'调节值不得小于 {MIN_MOD_NUMBER} 或大于 {MIN_MOD_NUMBER}')
...
class Dice(DiceItemBase):
    ...
    def __init__(self, diceCode: str,postive: bool):
        ...
        if self.type  <= 0:
            DiceValueError(f'骰子面数不得小于 2',self.count)
        if self.type  == 1:
            DiceValueError(f'1 ... 1 面的骰子？',self.count)
        if abs(self.adv) > self.count:
            DiceValueError(f'优劣势骰数大于总骰子数',self.adv)
...
async def GenerateMessage(dices: str,times: int, dc:int):
    ...
    for i in range(times):
        ...
        for dice in diceList:
            ...
            if dice is Dice and times * diceCount < MAX_DETAIL_CNT:
