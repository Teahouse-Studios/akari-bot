...
async def _(msg: Bot.MessageSession, sides: int):
    if sides > 1:
        await msg.session_info.target_union_info.edit_target_data("dice_default_sides", sides)
        await msg.finish(I18NContext("dice.message.set.success", sides=sides))
    elif sides == 0:
        await msg.session_info.target_union_info.edit_target_data("dice_default_sides", None)
...
async def _(msg: Bot.MessageSession):
    dc_rule = msg.session_info.target_union_info.target_data.get("dice_dc_reversed")

    if dc_rule:
        await msg.session_info.target_union_info.edit_target_data("dice_dc_reversed", False)
        await msg.finish(I18NContext("dice.message.rule.disable"))
    else:
        await msg.session_info.target_union_info.edit_target_data("dice_dc_reversed", True)
