...
如果同一个模型同时配置了 `tiers` 和 `time_rules`：
...
则 `tiers` 和 `time_rules` **均不生效**，不会按照优先级选择其中一种，也不会进行价格叠加。
...
`tiers` and `time_rules` **cannot be active at the same time**.

If both `tiers` and `time_rules` are configured for the same model:
...
then **neither `tiers` nor `time_rules` will take effect**. No priority is applied between them, and their prices will not be combined.

Only the base token prices will be used:
...
The following is a complete example using the new billing configuration:

```yaml
llm_api_list:
  # 1. Backward-compatible: simple token-based billing
  - api_key: sk-xxx
...
> **Note:** `tiers` and `time_rules` are both included in the complete example only to demonstrate their configuration formats. Since these two rules conflict with each other, neither will take effect when both are configured. In actual use, configure only one of them according to your requirements.
在使用本模块前，请确保在 `modules/ai/assets/` 目录下创建 `llm_api_list.yaml` 配置文件，否则模块将无法正常运行。
...
llm_api_list:
  - api_key: sk-1145141919810
    ...
    billing:
      ...
      # [可选] 每次 API 调用额外扣除的固定花瓣数量
      call_price: 10

      # Token 基础计费，单位：花瓣 / 1M token
      price_in: 50
      price_cache: 20
      price_out: 50
...
> **注意：** 上述完整示例中的 `tiers` 和 `time_rules` 是为了展示配置格式。由于两者存在冲突，同时配置时二者均不会生效。实际使用时，请根据需要只配置其中一种。
...
llm_api_list:
  - api_key: sk-1145141919810
    ...
    billing:
      ...
      # [Optional] Fixed number of petals charged for each API call
      call_price: 10

      # Base token pricing, unit: petals / 1M tokens
      price_in: 50
      price_cache: 20
      price_out: 50

      # [Optional] Tiered pricing based on context length
      tiers:
        - context_threshold: 131072
          price_in: 100
          price_out: 100
