...
def create_context(messages: list[dict], scene_key: str) -> str:
    """创建绑定场景的上下文快照，返回其 ID。"""
...
def get_context(session_id: str | None, scene_key: str) -> list[dict] | None:
    ...
    if not isinstance(window, ExpiringTempDict) or window.is_expired():
        return None
    if window.data.get("scene_key") != scene_key:
        return None
