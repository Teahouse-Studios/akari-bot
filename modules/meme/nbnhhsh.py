...
async def nbnhhsh(term: str, locale: Locale):
    ...
    try:
        ...
        if 'trans' in result:
            trans = result['trans']
            count = trans.__len__()
            return f'{locale.t("meme.message.nbnhhsh")} {locale.t("meme.message.nbnhhsh.result")}{"、".join(trans)}'
        elif 'inputting' in result and result['inputting'] != []:
            inputting = result['inputting']
            count = inputting.__len__()
            return f'{locale.t("meme.message.nbnhhsh")} {locale.t("meme.message.nbnhhsh.result.ai")}{"、".join(inputting)}'
        else:
            return f'{locale.t("meme.message.nbnhhsh")} {locale.t("meme.message.not_found")}'
    except Exception:
        traceback.print_exc()
