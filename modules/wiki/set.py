...
async def _(msg: Bot.MessageSession, wikiurl: str):
    ...
    if check.available:
        ...
        if result and enable_urlmanager and not in_allowlist:
            ...
            prompt = ""
        await msg.finish(
            msg.session_info.locale.t("wiki.message.set.success", name=check.value.name) + prompt
        )
    else:
...
async def _(msg: Bot.MessageSession, interwiki: str, wikiurl: str):
    ...
    if check.available:
        if (
            ...
            await msg.finish(
                msg.session_info.locale.t("wiki.message.invalid.blocked", name=check.value.name)
            )
...
async def _(msg: Bot.MessageSession, interwiki: str):
    ...
    if result:
        await msg.finish(msg.session_info.locale.t("wiki.message.iw.remove.success", iw=interwiki))
...
async def _(msg: Bot.MessageSession, headers: str):
    ...
    if add:
        ...
        await msg.finish(msg.session_info.locale.t("wiki.message.headers.add.failed"))
...
async def _(msg: Bot.MessageSession):
    ...
    if reset:
        await msg.finish(msg.session_info.locale.t("wiki.message.headers.reset.success"))
...
async def _(msg: Bot.MessageSession):
    ...
    if set_prefix:
        await msg.finish(msg.session_info.locale.t("wiki.message.prefix.reset.success"))
...
async def _(msg: Bot.MessageSession):
    ...
    if redlink_state:
        ...
        await msg.finish(msg.session_info.locale.t("wiki.message.redlink.disable"))
    else:
        await msg.session_info.target_info.edit_target_data("wiki_redlink", True)
        await msg.finish(msg.session_info.locale.t("wiki.message.redlink.enable"))
