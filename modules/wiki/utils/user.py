...
async def get_user_info(msg: Bot.MessageSession, wikiurl, username):
    ...
    if not await wiki.check_wiki_available():
        await msg.finish(msg.locale.t('wiki.message.user.wiki_unavailable', wikiurl=wikiurl))
    ...
    if user := data.get('username', False):
        msgs.append(msg.locale.t('wiki.message.user.username') + user + (
            ' | ' + msg.locale.t('wiki.message.user.edited_count') + data['edited_count']
            if 'edited_count' in data and 'created_page_count' not in data else ''))
    if users_groups := data.get('users_groups', False):
        msgs.append(
            msg.locale.t('wiki.message.user.users_groups') + msg.locale.t('message.delimiter').join(users_groups))
    ...
    if edited_wiki_count := data.get('edited_wiki_count', False):
        msgs.append(msg.locale.t('wiki.message.user.edited_wiki_count') + edited_wiki_count)

    sub_edit_counts1 = []
    if created_page_count := data.get('created_page_count', False):
        sub_edit_counts1.append(msg.locale.t('wiki.message.user.created_page_count') + created_page_count)
    if edited_count := data.get('edited_count', False) and created_page_count:
        sub_edit_counts1.append(msg.locale.t('wiki.message.user.edited_count') + edited_count)
    sub_edit_counts2 = []
    if deleted_count := data.get('deleted_count', False):
        sub_edit_counts2.append(msg.locale.t('wiki.message.user.deleted_count') + deleted_count)
    if patrolled_count := data.get('patrolled_count', False):
        sub_edit_counts2.append(msg.locale.t('wiki.message.user.patrolled_count') + patrolled_count)
    sub_edit_counts3 = []
    ...
    if global_rank := data.get('global_rank', False):
        sub_edit_counts3.append(msg.locale.t('wiki.message.user.global_rank') + global_rank)
    if sub_edit_counts1:
    ...
    if global_users_groups := data.get('global_users_groups', False):
        msgs.append(msg.locale.t('wiki.message.user.global_users_groups') + msg.locale.t('message.delimiter').join(
            global_users_groups))
    if global_edit_count := data.get('global_edit_count', False):
        msgs.append(msg.locale.t('wiki.message.user.global_edited_count') + global_edit_count)
    ...
    if blocked_by := data.get('blocked_by', False):
        ...
        msgs.append(msg.locale.t('wiki.message.user.blocked.detail',
                                 blocked_by=blocked_by,
                                 blocked_time=data['blocked_time'],
                                 blocked_expires=data['blocked_expires']))
        msgs.append(msg.locale.t('wiki.message.user.blocked.reason') + data['blocked_reason'])
