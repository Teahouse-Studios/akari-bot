...
async def make_songcard(
    ...
    if img_h < 240:
        ...
        img = downlight.enhance(0.5).resize((384, img_h)).crop((0, crop_start_y, 384, 240 + crop_start_y))
    else:
    ...
    songimg_width = 384
    drawtext.text((songimg_width - playtime_width - 20, 25), playtime, "#ffffff", font=font2)
    type_ = str(difficulty)
    type_text = Image.new("RGBA", (32, 32))
    draw_typetext = ImageDraw.Draw(type_text)
    draw_typetext.text(((32 - get_fontsize(font2, type_)[0]) / 2, 0), type_, "#ffffff", font=font2)
