...
async def _(msg: Bot.MessageSession):
    ...
    msg_list = [msg.session_info.locale.t("oba.message.status.detail",
    ...
    if version:
        msg_list.append(msg.session_info.locale.t("oba.message.status.version", version=version))
    msg_list.append(
...
async def _(msg: Bot.MessageSession, rank: int = 1):
    ...
    msg_list = [status, msg.session_info.locale.t("oba.message.node", name=name, id=_id, hits=hits, size=size)]
    msg_list.append(
        msg.session_info.locale.t(
            "oba.message.query_time",
            query_time=msg.format_time(
    ...
    if "sponsor" not in node:
        ...
        send_msg = msg.session_info.locale.t("oba.message.sponsor", name=name, url=url)
        try:
...
async def _(msg: Bot.MessageSession, rank: int = 1):
    ...
    for i in range(rank - 1, rank - 1 + TOP_LIMIT):
        node = rankList[i]
        sponsor = node.get("sponsor", msg.session_info.locale.t("message.unknown"))
        try:
            sponsor_name = sponsor.get("name")
        except AttributeError:
            sponsor_name = msg.session_info.locale.t("message.unknown")
        ...
        try:
            ...
            hits = node.get("metric").get("hits")
            size = size_convert(node.get("metric").get("bytes"))
            node_list.append(f"{status} | {msg.session_info.locale.t("oba.message.top",
                                                                     rank=i + 1,
                                                                     name=name,
                                                                     id=_id,
                                                                     hits=hits,
