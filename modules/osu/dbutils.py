...
class OsuBindInfoManager:
    @retry(stop=stop_after_attempt(3), reraise=True)
    @auto_rollback_error
    def __init__(self, msg: Bot.MessageSession):
        self.target_id = msg.target.sender_id
        self.query = session.query(OsuBindInfo).filter_by(targetId=self.target_id).first()
        if not self.query:
            session.add_all([OsuBindInfo(targetId=self.target_id, username='')])
            session.commit()
            self.query = session.query(OsuBindInfo).filter_by(targetId=self.target_id).first()
