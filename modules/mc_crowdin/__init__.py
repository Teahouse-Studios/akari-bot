...
#
#
# @mcr.schedule(IntervalTrigger(seconds=60 if not Config("slower_schedule", False) else 180))
...
#                         await Bot.post_message('mc_crowdin', message=MessageChain.assign([Embed(title="New Crowdin Updates", description=m)]))
#                 else:
...
#
#                             if not first and not await CrowdinActivityRecords.check(identify):
#                                 await Bot.post_message('mc_crowdin', MessageChain.assign([Embed(title="New Crowdin Updates", description=m, color=0x00ff00, fields=[EmbedField(name=k, value=v, inline=True) for k, v in identify_.items()])]))
#     except Exception:
