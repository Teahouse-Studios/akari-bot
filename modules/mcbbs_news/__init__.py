...
async def main(msg: MessageSession):
    def template(sth):
        return {"type": "node","data": {"name": f"MCBBS · 幻翼快讯","uin": qq_account,"content": [{"type": "text", "data": {"text": sth}}]}}
    message = ""
    nodelist = []
    response = await news()
    ...
    if msg.Feature.forward and msg.target.targetFrom == 'QQ|Group':
        try:
            for i in range(len(titles)):
                nodelist.append(template(f"标题：{title[i]}\n作者：{author[i]}\n发布日期：{time[i]}\n链接：{links[i]}"))
    ...
    if legacy: # 长得过分，所以截取5条
        ...
        await msg.send(message[0:-1]) # 最后会多一个换行符
