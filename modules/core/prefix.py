    ...
    if 'add' in msg.parsed_msg:
        if arg1:
            if arg1 not in prefixes:
                ...
                await msg.sendMessage(msg.locale.t("core.message.prefix.add.success", prefix=arg1))
            else:
                await msg.sendMessage(msg.locale.t("core.message.prefix.add.already"))
    elif 'remove' in msg.parsed_msg:
        if arg1:
            if arg1 in prefixes:
                prefixes.remove(arg1)
                msg.data.edit_option('command_prefix', prefixes)
                await msg.sendMessage(msg.locale.t("core.message.prefix.remove.success") + arg1)
            else:
                await msg.sendMessage(msg.locale.t("core.message.prefix.remove.not_found"))
    elif 'reset' in msg.parsed_msg:
        msg.data.edit_option('command_prefix', [])
        await msg.sendMessage(msg.locale.t("core.message.prefix.reset"))
    elif 'list' in msg.parsed_msg:
        if len(prefixes) == 0:
            await msg.sendMessage(msg.locale.t("core.message.prefix.list.none"))
        else:
