...
@petal_.command("give <user> <petal> {[I18N:core.help.petal.give]}")
async def _(msg: Bot.MessageSession, user: str, petal: int):
    if petal <= 0:
...
@petal_.command(["[<sender>]",
                 "modify <petal> [<sender>]",
                 "clear [<sender>]"],
                required_superuser=True)
async def _(msg: Bot.MessageSession):
    sender = msg.parsed_msg.get("<sender>", False)
    if msg.parsed_msg.get("modify", False):
        ...
        if sender:
            if not any(sender.startswith(f"{sender_from}|") for sender_from in sender_list):
                await msg.finish(I18NContext("message.id.invalid.sender", sender=msg.target.sender_from))
            sender_info = await SenderInfo.get_by_sender_id(sender)
            await sender_info.modify_petal(petal)
        ...
        if sender:
            if not any(sender.startswith(f"{sender_from}|") for sender_from in sender_list):
                await msg.finish(I18NContext("message.id.invalid.sender", sender=msg.target.sender_from))
            sender_info = await SenderInfo.get_by_sender_id(sender, create=False)
            if not sender_info:
                if not await msg.wait_confirm(I18NContext("message.id.init.sender.confirm"), append_instruction=False):
                    await msg.finish()
                await SenderInfo.create(sender_id=sender)
