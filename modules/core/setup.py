...
@setup.command("invalid {{I18N:core.help.setup.invalid}}", required_admin=True)
async def _(msg: Bot.MessageSession):
    if not msg.session_info.invalid_module_prompt_enabled:
        ...
        await msg.finish(I18NContext("core.message.setup.invalid.enable"))
    else:
        await msg.session_info.target_union_info.edit_target_data("invalid_module_prompt", False)
        await msg.finish(I18NContext("core.message.setup.invalid.disable"))
...
def build_target_rows(msg: Bot.MessageSession) -> list[SettingRow]:
    ...
    rows += [
        ...
        _toggle_row(
            locale,
            "core.message.setup.list.item.invalid",
