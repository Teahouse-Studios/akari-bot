...
async def get_weekly(with_img=False, zh_tw=False):
    ...
    imglink = None
    if img_filename:
        get_image = await WikiLib("https://zh.minecraft.wiki/").parse_page_info(
            img_filename.group(1)
        )
        if get_image.status:
        ...
        Plain(
            locale.t(
                ...
                article=str(
                    ...
                    if page
                    else locale.t("message.none")
                ),
                link=str(
                    Url(
                        f'https://zh.minecraft.wiki/wiki/?oldid={str(result["parse"]["revid"])}'
                    )
                ),
            )
        )
...
async def get_weekly_img(with_img=False, zh_tw=False):
    ...
    img = await generate_screenshot_v2(
        "https://zh.minecraft.wiki/wiki/Minecraft_Wiki/"
        + ("?variant=zh-tw" if zh_tw else ""),
        content_mode=False,
...
async def _(msg: Bot.MessageSession):
    weekly = await get_weekly(
        msg.target.client_name in ["QQ", "TEST"], zh_tw=msg.locale.locale == "zh_tw"
    )
