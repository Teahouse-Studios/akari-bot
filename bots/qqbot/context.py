...
async def _upload_markdown_image(client: botpy.Client, target: ReplyTarget, *, local_path: str) -> str | None:
    ...
    ``Client.upload_media`` 只在文件达到 5 MiB 时启用分片，而普通上传响应没有
    ``raw_url``。这里复用 botpy 的分片实现并强制走该路径；QQ 返回没有直链时交给
    调用方继续尝试 S3 或普通消息降级。
    """
    if target.scope not in ("group", "c2c"):
        return None
    # 兼容测试替身和旧版 botpy；真实客户端始终提供 api。
    ...
    # COS 直链未必声明图片 MIME，补上响应类型可避免客户端按附件处理。
...
def _api_error_codes(error: ApiError) -> set[str]:
    """收集 QQ OpenAPI 异常在兼容层和响应正文中的错误码。"""
    codes = [error.code]
        ...
        async def prepare_markdown_message() -> _PreparedMessage:
            ...
            for x in converted_message:
                if isinstance(x, PlainElement):
                    ...
                    if image_path is not None:
                        try:
                            # QQ 分片上传返回的 raw_url 是短期有效的临时直链，足以
                            # 支撑本次 Markdown 消息，不再强制依赖 S3 图床。
                            try:
