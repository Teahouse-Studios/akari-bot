...
async def gen(app, message, target1, target2='0', msgtype='None'):
    ...
    if run != None:
        ...
        if msgtype == 'friend':
            ...
            group = target1
            member = target2
            send = await app.sendGroupMessage(group, msgchain.asSendable(), quote=message['Source'][0].id)
        elif msgtype == 'temp':
            group = target1
            member = target2
...
async def findwikitext(app, message, target1, target2='0', msgtype='None'):
    ...
    if str(z) == '['']['']' or str(z) == '[][]' or str(z) == '[]':
        ...
        if msgtype == 'friend':
            ...
            group = target1
            member = target2
            await app.sendGroupMessage(group, msgchain.asSendable(), quote=message['Source'][0].id)
        elif msgtype == 'temp':
            group = target1
            member = target2
