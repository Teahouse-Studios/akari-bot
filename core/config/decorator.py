...
该模块提供了一个配置装饰器，用于将Python类自动转换为配置对象。
...
def _process_class(cls: type[T], table_name, secret=False) -> type[T]:
    ...
    该函数是核心的转换逻辑，它会：
    1. 提取类的所有注解（类型提示）
    2. 为类生成自定义的__init__和__repr__方法
    3. 自动生成和管理配置文件

    Args:
        cls: 要处理的类对象
        table_name: 配置表名称，用于在配置文件中标识该配置块
        secret: 是否将该配置的值视为敏感信息进行加密存储（默认False）

    Returns:
        处理后的类对象，具有自动生成的初始化和字符串表示方法
    """
    # 获取类的所有类型注解（排除私有属性）
    ...
    def __generate_config_file():
        ...
        处理的类型包括：
        - ALLOWED_TYPES中定义的基本类型
        - Union类型（只要所有分支都在ALLOWED_TYPES中）
        """
        for attr_name, attr_type in cls_annotations.items():
            ...
            if not attr_name.startswith("__"):
                ...
                # 检查类型是否在允许列表中
                # 对于Union类型（如 str | int），需要检查其所有分支是否都被允许
                # 如果类型不被允许，则将其设置为None
...
def on_config(table_name: str, table_type: Literal["module", "bot", ""] = "", secret: bool = False):
    ...
    该装饰器会自动：
    1. 为MyConfig类生成__init__和__repr__方法
    2. 在配置文件中创建"module_my_config"表
    3. 为所有类属性创建配置项

    Args:
        table_name: 配置表的基本名称。最终的表名为"table_type_table_name"的形式
                   例如：table_type="module", table_name="myconfig" -> "module_myconfig"
        table_type: 配置表的类型，用于分类和命名空间隔离（默认""）
                   - "module": 模块配置
                   - "bot": 机器人配置
                   - "": 空字符串表示不添加前缀
        secret: 是否将此配置中的所有值视为敏感信息进行加密存储（默认False）
               设置为True时，配置值会被加密存储在配置文件中

    Returns:
        装饰器函数，接收一个类并返回处理后的类

    Raises:
        无，但如果table_type不是指定的字面值之一，在类型检查时会有警告
    """
    ...
    def wrap(cls: type[T]):
        ...
        Returns:
            处理后的类，具有自动生成的配置管理功能
        """
        # 构造表名：如果table_type不为空，则添加前缀和下划线分隔符
...
# 将_process_class函数导出到系统模块导出表中，使其可被其他模块导入使用
