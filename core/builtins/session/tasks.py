...
class SessionTaskManager:
    ...
    # 存储活跃任务的字典，按消息通道、用户 union、消息会话层次组织。
    # 场景维度用消息通道而非 union：仅共享 union 而通道号不同的场景是不同的现实场景，
    # 一边的等待任务不应被另一边的消息误命中。
    _task_list = {}
    ...
    def add_task(
        ...
        # 创建必要的嵌套字典结构
        if msg.session_info.channel_key not in cls._task_list:
            cls._task_list[msg.session_info.channel_key] = {}
        if sender not in cls._task_list[msg.session_info.channel_key]:
            cls._task_list[msg.session_info.channel_key][sender] = {}
        ...
        # 存储任务信息
        cls._task_list[msg.session_info.channel_key][sender][msg] = {
            "flag": flag,  # 同步事件标志
    ...
    def get_result(cls, msg: "MessageSession"):
        ...
        """
        # 检查是否存在任务结果
        if "result" in cls._task_list[msg.session_info.channel_key][msg.session_info.sender_union_id][msg]:
            return cls._task_list[msg.session_info.channel_key][msg.session_info.sender_union_id][msg]["result"]
        return None
    ...
    async def check(cls, session: "MessageSession"):
        ...
        """
        # 检查是否有任务在等待此消息
        if session.session_info.channel_key in cls._task_list:
            # 收集所有相关的用户（该用户 + "all" 广播）
            senders = []
            if session.session_info.sender_union_id in cls._task_list[session.session_info.channel_key]:
                senders.append(session.session_info.sender_union_id)
            if "all" in cls._task_list[session.session_info.channel_key]:
                senders.append("all")
