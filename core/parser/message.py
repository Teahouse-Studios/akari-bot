...
async def parser(message: dict):
    ...
    :return: 无返回
    """
    display = RemoveDuplicateSpace(Template.asDisplay(message))  # 将消息转换为一般显示形式
    ...
    if display[0] in command_prefix:  # 检查消息前缀
        ...
        if len(command_list) > 5 and not senderInfo.query.isSuperUser:
            await Template.sendMessage(message, '你不是本机器人的超级管理员，最多只能并排执行5个命令。')
            return
                ...
                if command_first_word in Modules:  # 检查触发命令是否在模块列表中
                    ...
                    if module.is_superuser_function:
                        if not senderInfo.query.isSuperUser:
                            await Template.sendMessage(message, '你没有使用该命令的权限。')
                    ...
                    if not module.is_base_function:
                        ...
                        if not check_command_enable:  # 若未开启
                            await Template.sendMessage(message, f'此模块未启用，请管理员在群内发送~enable {command_first_word}启用本模块。')
                            return
                    ...
                    if help_doc is not None:
                        try:
                            ...
                            try:
                                ...
                                await Template.sendMessage(message, f'语法错误。\n' + cparser.return_formatted_help_doc())
                        except InvalidHelpDocTypeError:
                            await Template.sendMessage(message, f'此模块的帮助信息有误，请联系开发者处理。')
                    await Modules[command_first_word].function(message)  # 将dict传入下游模块
            except Exception as e:
                traceback.print_exc()
                await Template.sendMessage(message, '执行命令时发生错误，请报告管理员：\n' + str(e))
