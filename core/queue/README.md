...
`node_id` 是可配置的节点标签。`configure_peer()` 未显式传入 `node_id` 时，运行时读取
`[jobqueue].jobqueue_node_id`；该配置项不存在或尚未填写时，将在每次进程生命周期内分别生成新的 UUID。
运行时不会从计算机名、宿主机环境或其它设备信息中读取或推导该值，也不会持久化自动生成的 UUID。因此，
默认情况下可使用 `PeerSelector.node()` 精确选择本次进程实例，但不应将自动生成的 `node_id` 解释为稳定的
物理节点标识。

若需要按物理节点或部署单元选择一组 Peer，可为共用同一份配置的相关进程设置相同且不包含敏感信息的
`jobqueue_node_id`，也可在调用 `configure_peer()` 时显式传入 `node_id`。身份来源的优先级依次为显式参数、
配置项和自动生成的 UUID。
配置值最长为 128 个字符，修改后须重启相关进程才会反映到 Peer Registry。
