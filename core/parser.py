...
async def parser(kwargs: dict):
    ...
    #        if not BotDBUtil.check_superuser(kwargs):
    ...
    #            return
        for command in command_list:
            ...
            try:
                ...
                kwargs['bot_aliases'] = ModulesAliases
                ...
                if command_first_word in Modules:  # 检查触发命令是否在模块列表中
                    ...
                    if not module.is_base_function:
                        check_command_enable = BotDBUtil.Module(kwargs).check_target_enabled_module(command_first_word)  # 检查群组是否开启模块
