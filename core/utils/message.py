...
def chunk_list(iterable: Iterable[Any], chunk_size: int) -> Generator[List[Any], None, None]:
    """将可迭代对象分块，返回生成器。"""
...
def unique_list(iterable: Iterable[Any]) -> List[Any]:
    """在不破坏顺序下去重可迭代对象。"""
    if isinstance(iterable, (str, bytes)):
        raise TypeError("Type str is not supported")
...
def snake_to_camel(text: str, upper: bool = True) -> str:
    """蛇形命名转驼峰命名。
