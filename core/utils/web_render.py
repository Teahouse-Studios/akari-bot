...
def webrender(method: str = '', url: str = '', use_local: bool = True, ignore_status=False) -> Union[str, None]:
    ...
    '''
    if use_local and (not Info.web_render_local_status or ignore_status):
        use_local = False
    if method == 'source':
...
async def check_web_render() -> Tuple[bool, bool]:
    ...
    if web_render_status:
        try:
            ...
            await get_url(webrender('source', ping_url, ignore_status=True), 200, request_private_ip=True)
